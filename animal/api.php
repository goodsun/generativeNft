...
function generateSVGImage($animal, $item, $colorScheme, $effect) {
    // SVGファイルを読み込む
    $animalSVG = file_get_contents(__DIR__ . '/assets/animals/' . $animal['file']);
    $itemSVG = file_get_contents(__DIR__ . '/assets/items/' . $item['file']);
    
    // SVGタグを除去
    $animalSVG = preg_replace('/<\/?svg[^>]*>/', '', $animalSVG);
...
function generateMetadataById($id) {
    global $animals, $items, $colorSchemes, $effects;
    
    $seed = intval($id) ?: 0;
    
    $animal = getSeededElement($animals, $seed, 1);
    ...
    $name = $colorScheme['name'] . ' ' . $animal['name'] . ' #' . $id;
    $description = 'A unique pixel art ' . strtolower($animal['name']) . ' with ' . strtolower($item['name']) . ' in ' . strtolower($colorScheme['name']) . ' color scheme. ' . $rarity . ' collectible from the Pixel Animals collection.';
    
    // SVG画像を生成
    $image = generateSVGImage($animal, $item, $colorScheme, $effect);
